...
async def test_discussion(
    ...
    test_messages = [
        MessageDB(
            text=["Test", "Message", "This is test msg", 
                "Just test", "Msg Test"][i % 5],
...
class TestDiscussion:
    async def test_create_discussion_and_message(
        ...
        diss_response = await auth_client.post(
            ...
            json={
            "topic": "This app is questionable"
                }
        )
    ...
    async def test_create_discussion_app_not_exists(
        ...
        response = await auth_client.post(
            f"/api/v1/discussions/097c51bc-3c31-4cdf-b726-a4b1df084d8e",
            json={
            "topic": "test topic"
                }
        )
    ...
    async def test_delete_discussion_not_exists(
        ...
        delete_response = await auth_client.delete(
            f"/api/v1/discussions/399848f1-a846-4f82-ac3e-aa3caf7394ee"
        )
...
class TestMessage:
    async def test_create_message(
        ...
        msg_response = await auth_client.post(
            ...
            json={
                "text": "Another test message for a test discussion."
            }
        )
