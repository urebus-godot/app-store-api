...
class FileService:
    ...
    async def presign_app_archive_download(
        ...
        async with self._uow:
            ...
            app = await self._uow.games.get(app_id)
    ...
    async def presign_app_cover_upload(
        self,
        *,
        app_id: uuid.UUID,
        publisher_id: uuid.UUID,
        ...
        async with self._uow:
            ...
            if app.pending_cover_key is not None:
                raise HTTPException(
                    status.HTTP_404_NOT_FOUND,
                    "Did not confirmed previous cover key"
                )
            ...
            # загрузил файл. Если аплоад оборвётся — останется висячая
            # запись, её можно подчищать periodic Celery-таской.
            app.pending_cover_key = object_key
            await self._uow.commit()
    ...
    async def confirm_app_cover_upload(
        ...
        *, 
        ...
        async with self._uow:
            app = await self._uow.app_repo.get_app(app_id)
            if app.publisher_id != publisher_id or app.pending_cover_key is None:
                raise HTTPException(
                    status.HTTP_404_NOT_FOUND, 
                    "Нет ожидающей загрузки"
                    )
            ...
            # поэтому не верим клиенту на слово, а перепроверяем head_object
            exists = await self._storage.object_exists(
                settings.APP_COVER_BUCKET, app.pending_archive_key
            )
            if not exists:
                raise HTTPException(
                    status.HTTP_409_CONFLICT, 
            ...
            app.cover_keys.append(app.pending_cover_key)
            app.pending_cover_key = None
            await self._uow.commit()
